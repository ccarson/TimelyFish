...
-- Description:	this gives the number of pig deaths by PIC week/pig group
-- *source data from jmaas
-- ==================================================================
CREATE VIEW [dbo].[cfv_PIG_GROUP_CENSUS_PIG_DEATHS]
AS
select
	pg.PigGroupID,
	pg.PigProdPhaseID,
...
left join [$(SolomonApp)].dbo.cftPGInvTran it (nolock)
	on ((it.TranTypeID = 'GD' and it.TranDate<=d.WeekEndDate)
	and (it.TranTypeID = 'GD' and it.TranDate>=d.WeekOfDate))
