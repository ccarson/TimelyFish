...
using SystemUtils = QRCode.Helper.SystemUtils;
...
namespace QRCode
{
    public class QRCodeEncoder
    {
        ...
        virtual public String CharacterSet
        {
            get;
            set;
        }
		...
		private static sbyte[] CalculateRSECC(sbyte[] codewords, sbyte rsEccCodewords, sbyte[] rsBlockOrder, int maxDataCodewords, int maxCodewords)
		{
			...
			try
			{
			    ...
			    StreamReader reader = new StreamReader(filename);
                BufferedStream bis = new BufferedStream(reader.BaseStream);
                //String fileName = DATA_PATH + "." + "rsc" + rsEccCodewords.ToString() + ".dat";
                //MemoryStream memoryStream = new MemoryStream(Resources.GetResource(fileName));
                //Stream stream = GetEmbeddedFile(fileName);
                //BufferedStream bis = new BufferedStream(stream);
				for (int i = 0; i < 256; i++)
        ...
        {
            return Encode(content, Encoding.GetEncoding(this.CharacterSet));
            /*
            if (QRCodeHelper.IsUniCode(content))
            {
        ...
        {
            string assemblyName = typeof(QRCodeEncoder).Assembly.GetName().Name;
            try
            {
            ...
            {
                throw new Exception(assemblyName +"." +fileName + ": " + e.Message);
            }
        }
