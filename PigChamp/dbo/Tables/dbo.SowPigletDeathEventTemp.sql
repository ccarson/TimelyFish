CREATE TABLE [dbo].[SowPigletDeathEventTemp] (
    ...
    [SortCode]    INT           NULL,
    CONSTRAINT [PK_SowPigletDeathEventTemp] PRIMARY KEY CLUSTERED ([EventID] ASC) WITH (FILLFACTOR = 80)
);


GO
CREATE NONCLUSTERED INDEX [idxSowPigletDeathEventTemp_FarmIDSowID]
    ON [dbo].[SowPigletDeathEventTemp]([FarmID] ASC, [SowID] ASC) WITH (FILLFACTOR = 80);


GO
CREATE NONCLUSTERED INDEX [idxSowPigletDeathEventTemp_WeekOfDate]
    ON [dbo].[SowPigletDeathEventTemp]([WeekOfDate] ASC) WITH (FILLFACTOR = 80);
