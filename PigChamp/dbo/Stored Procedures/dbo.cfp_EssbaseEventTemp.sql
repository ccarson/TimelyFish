...
CREATE PROC [dbo].[cfp_EssbaseEventTemp]
	AS
...
INSERT INTO SowFarrowEventTemp
		...
		FROM caredata.CFV_SowFarrowEvent WHERE EventDate >=  
...
--Insert all transport records for the date range user selected
	INSERT INTO SowMatingEventTemp
		SELECT eventid, farmid, sowid, matingtype, eventdate, weekofdate, semenid, observer, hourflag, matingnbr, sowparity, sowgenetics
		 FROM caredata.cfv_SowMatingEvent WHERE EventDate >=  
			(select min(weekofdate) from weekdefinitiontemp)
...
Begin Transaction
	INSERT INTO SowweanEventTemp
		SELECT eventid, farmid, sowid, eventtype, eventdate, weekofdate, qty, sowparity, sowgenetics 
		FROM caredata.cfv_SowweanEvent WHERE EventDate >=  (select min(weekofdate) from weekdefinitiontemp)
Commit

Begin Transaction
	INSERT INTO SownurseEventTemp
		SELECT eventid, farmid, sowid, eventtype, eventdate, weekofdate, qty, sowparity, sowgenetics, null 
		FROM caredata.cfv_SownurseEvent WHERE EventDate >=  (select min(weekofdate) from weekdefinitiontemp)
Commit
...
Begin Transaction
	insert into SowPigletDeathEventTemp
		...
		from caredata.cfv_SowPigletDeathEvent
Commit
