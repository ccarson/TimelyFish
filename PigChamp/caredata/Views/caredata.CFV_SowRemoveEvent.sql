...
CREATE VIEW [caredata].[CFV_SowRemoveEvent]
AS
SELECT  IH.identity_id
	  ...
	  , rd.[creation_date] as entry_creation_date
  ...
  inner join [caredata].[bh_events] rd (NOLOCK) on IH.identity_id = rd.identity_id and IH.site_id = rd.[site_id] and rd.[event_type] in (298,299,300,301) and rd.[deletion_date] is NULL
  left join dbo.cft_sowparity sp (nolock) on IH.[identity_id] = sp.[identity_id] and IH.site_id = sp.[site_id] 
		and rd.eventdate between sp.eventdate and isnull(sp.enddate,getdate()) 	
