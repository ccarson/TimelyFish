...
CREATE VIEW [caredata].[CFV_SowWeanEvent_orig]
AS
      ...
      , sp.Parity + coalesce(sowHdr.[starting_parity],0) as SowParity
  ...
  left join dbo.cft_SowParity sp (nolock)
	on IH.identity_id = sp.identity_id and IH.site_id = sp.site_id and ev.eventdate >= sp.eventdate and ev.eventdate < isnull(sp.enddate,getdate())
  ...
  --inner join [caredata].[HDR_SOWS] sowHdr (NOLOCK) on IH.[identity_id] = sowHdr.[identity_id]
  --left join [caredata].[GENETICS] gen (NOLOCK) on sowHdr.[genetics_id] = gen.[genetics_id]
  --inner join caredata.bh_events ev (NOLOCK) on IH.identity_id = ev.identity_id and ev.[event_type] in (220,265) and ev.[deletion_date] is NULL
  --left join [caredata].[EV_WEANINGS] wean (NOLOCK) on ev.identity_id = wean.[identity_id] and ev.[event_id] = wean.[event_id]
