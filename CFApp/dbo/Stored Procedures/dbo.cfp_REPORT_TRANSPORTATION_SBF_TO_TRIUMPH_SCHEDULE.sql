...
-- =============================================
-- Author:	Matt Dawson/Brian Diehl
-- Create date:	1/2/2008:2/3/2016
-- Description:	FULL Transportation Schedule modified to be SBF to Triumph schedule = should return no rows.
-- Parameters: 	@StartDate, (allow null values)
--		@EndDate, (allow null values)
...
-- =============================================
CREATE PROCEDURE [dbo].[cfp_REPORT_TRANSPORTATION_SBF_TO_TRIUMPH_SCHEDULE] 
	@StartDate DATETIME,
	@EndDate DATETIME
AS
--drop table #Schedule
...
--set @StartDate='20160131'
--set @EndDate='20160206'

Declare @PMTypeString VARCHAR(2), @PMSystemString VARCHAR(50)
Set @PMTypeString='02'
set @PMSystemString='00'
...
BEGIN
	SET @StartDate=(select GETDATE())
END

IF @EndDate is null 
BEGIN
	SET @EndDate=(select dateadd(d,14,GETDATE()))
END
...
Inner JOIN [$(SolomonApp)].dbo.cftPigGroup on cftpiggroup.piggroupid=cftpm.sourcepiggroupid and cftpiggroup.PigProdPodID = 53 -- Seaboard
...
and cftpm.DestContactID=''002936''
ORDER BY
	cftPM.MovementDate,
	Case when LEN(RTRIM(cftPM.LoadingTime))>0 
		then CONVERT(CHAR(19),cftPM.LoadingTime,108)
...
,	DestRoomNbr
,	DisinfectFlg
,	ltrim(rtrim(Trucker))+ltrim(rtrim(isnull(Trailer,' '))) as Trucker
--,	Trailer
,	PICWeek
,	ScheduleStatus
,	WeekOfDate
...
,   PigGroupID
FROM #Schedule
ORDER BY MovementDate,LoadTime
