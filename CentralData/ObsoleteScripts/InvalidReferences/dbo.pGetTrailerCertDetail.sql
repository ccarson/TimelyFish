...
Select distinct 
	...
	PrintFlag=Case 
			when (SourceTestStatus in(0,'') and DestTestStatus in (0,'') and (ti.PassFail=0) and Walkthrough in (0,1)) 
				and (cast(pm.MovementDate + ' ' + pm.LoadingTime as smalldatetime) between ti.CertDateTime and DateAdd(d,5,ti.CertDateTime)) and isnull(ti.PrintFlg,0)=0 then 1
			...
			when (pm.PigTypeID='04' or pm.PigTypeID='05') and (ti.PassFail=0) and isnull(ti.PrintFlg,0)=0
				and (cast(pm.MovementDate + ' ' + pm.LoadingTime as smalldatetime)  between ti.CertDateTime and DateAdd(d,5,ti.CertDateTime)) then 1
			when isnull(ti.PrintFlg,0)=-1 and ((SourceTestStatus in(0,'') and DestTestStatus in (0,'') and (ti.PassFail=0) and Walkthrough in (0,1))
					or (pm.PigTypeID='04' or pm.PigTypeID='05') and (ti.PassFail=0)) then 2 
			else 0
			End
...
LEFT JOIN dbo.vEmployeewithContactName e on ti.InspectorUserName=e.UserID

where isnull(pm.Highlight,0)<>255
...
and TrailerWashFlag=-1 and PMSystemID='01'
