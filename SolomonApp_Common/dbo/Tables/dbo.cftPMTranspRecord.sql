CREATE TABLE [dbo].[cftPMTranspRecord] (
    ...
    [LoadWeighed]      SMALLINT      CONSTRAINT [DF_cftPMTranspRecordx_LoadWeighed] DEFAULT ((0)) NOT NULL,
    ...
    CONSTRAINT [cftPMTranspRecord0] PRIMARY KEY CLUSTERED ([BatchNbr] ASC, [RefNbr] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE NONCLUSTERED INDEX [IX_cftPMTranspRecord_DestPigGroupID]
    ON [dbo].[cftPMTranspRecord]([DestPigGroupID] ASC) WITH (FILLFACTOR = 100);


GO
CREATE NONCLUSTERED INDEX [IX_cftPMTranspRecord_SourcePigGroupID]
    ON [dbo].[cftPMTranspRecord]([SourcePigGroupID] ASC) WITH (FILLFACTOR = 100);
