CREATE PROCEDURE WSL_AssignableEmployeeList
 @page  int
 ,@size  int
 ,@sort   nvarchar(200)
		...
		SET @STMT = 
			'SELECT employee [Employee], 
			 ...
			 WHERE employee LIKE ' + quotename(@parm1,'''') + '
			   AND ((PJEMPLOY.emp_status = ''A'') or (PJEMPLOY.emp_status = ''I'' and PJEMPLOY.Placeholder=''Y'')) 
			 ORDER BY ' + @sort
	  END		 
  ELSE
	  BEGIN
			...
			SET @STMT = 
				...
				SELECT employee [Employee], emp_name [Name], manager1 [Manager1], cpnyid [Company], Subcontractor [Subcontractor], MSPType [Resource Type], placeholder [Generic], 
			           emp_status [Status], em_id21 [Location], em_id22 [Skill1], em_id23 [Skill2], em_id24 [Level], em_id25 [License/Cert]
				FROM PagingCTE                     
