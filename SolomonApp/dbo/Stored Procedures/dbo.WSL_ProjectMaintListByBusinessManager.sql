...
CREATE PROCEDURE [dbo].[WSL_ProjectMaintListByBusinessManager]
 @page  int
 ,@size  int
 ,@sort   nvarchar(200)
 ...
 ,@parm4 varchar(12) -- Business Manager ID
AS
  SET NOCOUNT ON
  DECLARE
    @STMT nvarchar(max), 
	  ...
	  BEGIN
		...
		SET @STMT = 
			'SELECT distinct QQ_pjprojNamed.project, [project description], isnull([customer name],'''') as [customer name], [project mgr name], isnull([business mgr name],'''') as [business mgr name], [company], [start date], [end date], [project status], QQ_pjprojNamed.noteid, Comment
			 ...
			 [business manager] LIKE ' + quotename(@parm4,'''') + '
			 ORDER BY ' + @sort
	  END		 
  ELSE
	  BEGIN
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[WSL_ProjectMaintListByBusinessManager] TO [MSDSL]
    AS [dbo];
