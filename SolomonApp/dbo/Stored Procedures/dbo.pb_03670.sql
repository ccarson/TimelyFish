...
CREATE PROCEDURE pb_03670 @RI_ID SMALLINT
...
SELECT @RI_Where = LTRIM(RTRIM(RI_Where)), @BegPerNbr = BegPerNbr, @EndPerNbr = EndPerNbr, @ViewName=ReportName
FROM RptRunTime
WHERE RI_ID = @RI_ID

SELECT @Search = "({vr_" + Rtrim(@ViewName) + ".cRI_ID} = " + RTRIM(CONVERT(VARCHAR(6),@RI_ID)) + 
                  " OR {vr_" + Rtrim(@ViewName) + ".cRI_ID} = 0)"
