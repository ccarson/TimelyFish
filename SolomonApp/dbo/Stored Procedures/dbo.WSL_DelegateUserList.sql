CREATE PROCEDURE WSL_DelegateUserList
  @page  int
 ,@size  int
 ,@sort   nvarchar(200)
		...
		SET @STMT = 
			'SELECT  Employee,  
				 CASE
					  ...
					  ELSE emp_name
				 END [Name],
			 cpnyid [Company], projExec [Executive Flag], user_id [UserID]
			 FROM PJEmploy (nolock)
			 where employee like ' + quotename(@parm1,'''') + '
			  and user_id <> '''' and emp_status = ''A'' 
			 ORDER BY ' + @sort
	  END		 
  ELSE
	  BEGIN
			...
			SET @STMT = 
				...
				SELECT  Employee, emp_name [Name], cpnyid [Company], projExec [Executive Flag], user_id [UserID]
				FROM PagingCTE                     
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[WSL_DelegateUserList] TO [MSDSL]
    AS [dbo];
