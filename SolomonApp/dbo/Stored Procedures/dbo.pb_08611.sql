 CREATE PROCEDURE pb_08611 @RI_ID SMALLINT
...
SELECT @RI_Where = LTRIM(RTRIM(RI_Where)), @BegPerNbr = BegPerNbr, @EndPerNbr = EndPerNbr, @Search = RTRIM(ReportName)
FROM RptRunTime
WHERE RI_ID = @RI_ID

SELECT @Search = "( {vr_08611.cRI_ID} = " + RTRIM(CONVERT(VARCHAR(6),@RI_ID)) + ")"
