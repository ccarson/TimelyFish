 CREATE PROCEDURE EDSoShipHeader_EDIInvToSend2ConsolInv1  AS
...
Select MIN(s.AccrDocDate) AccrDocDate, MIN(s.AccrPerPost) AccrPerPost, MIN(s.AccrRevAcct) AccrRevAcct,
       ...
       sg.CpnyID, MIN(s.CreditApprDays) CreditApprDays,
       ...
       MIN(s.CuryTotFrtCost) CuryTotFrtCost, Sum(sg.CuryTotFrtInvc) CuryTotFrtInvc, Sum(sg.CuryTotInvc) CuryTotInvc,
       ...
       sg.ShipRegisterID, MIN(s.ShipSiteID) ShipSiteID, MIN(s.ShipState) ShipState,
 ...
 WHERE ltrim(rtrim(sg.shipregisterid)) <> ''
   AND sg.EDI810 <> 0
...
Group by sg.InvcNbr,sg.CustOrdNbr,sg.ShiptoID,sg.CustID,sg.CpnyID,sg.ShipViaID,sg.shipregisterid



GO
GRANT CONTROL
    ON OBJECT::[dbo].[EDSoShipHeader_EDIInvToSend2ConsolInv1] TO [MSDSL]
    AS [dbo];
