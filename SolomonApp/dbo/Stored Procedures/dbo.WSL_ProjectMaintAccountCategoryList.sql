...
CREATE PROCEDURE [dbo].[WSL_ProjectMaintAccountCategoryList]
 @page  int
 ,@size  int
 ,@sort   nvarchar(200)
...
AS
  ...
  IF @page = 0  -- Don't do paging
	  BEGIN
		SET @STMT = 
			'SELECT PJACCT.acct [' + @AcctAlias + '], acct_type [' + @TypeAlias + '], acct_desc [' + @DescrAlias + ']
			 FROM PJACCT (nolock), PJPROJMX (nolock)
			 WHERE PJACCT.acct LIKE ' + quotename(@parm1,'''') + @whereExpression + '
			 AND pjprojmx.acct = pjacct.acct and project = ''na'' and pjt_entity = ''na''
			   AND acct_status = ''A'' 
			 ORDER BY ' + @sort
	  END		 
  ELSE
	  BEGIN
			...
			SET @STMT = 
				'WITH PagingCTE AS
				(
				SELECT TOP(' + CONVERT(varchar(9), @ubound-1) + ') PJACCT.acct, acct_type, acct_desc
				,ROW_NUMBER() OVER(
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[WSL_ProjectMaintAccountCategoryList] TO [MSDSL]
    AS [dbo];
