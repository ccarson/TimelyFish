...
JOIN SolomonApp_dw.dbo.cft_PIG_GROUP_ROLLUP cft_PIG_GROUP_ROLLUP
...
FROM SolomonApp_dw.dbo.cft_PIG_GROUP_ROLLUP (NOLOCK)
...
SET 	AverageMarket_Wt = 
		CASE WHEN HeadSold = 0
			...
			ELSE (Prim_Wt + Cull_Wt + DeadPigsToPacker_Wt + TransferToTailender_Wt + TransferOut_Wt) / HeadSold
		END
FROM SolomonApp_dw.dbo.cft_PIG_MASTER_GROUP_ROLLUP (NOLOCK)
