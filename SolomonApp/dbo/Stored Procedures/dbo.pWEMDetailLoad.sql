...
CREATE   Procedure [dbo].[pWEMDetailLoad]
AS
...
isnull([User ID],'WEM_001327'), [ts], ISNULL([DeliveryDate],'1/1/1900'), [Gross], [FeedLoadInfinityRecNo], [Miles], [Tare], -- 20141002 smr had to convert nulls to something, SL does not allow nulls
[TruckNo], [DriverNo], [InvoiceNo], [ScaleOperator], [Void], [Processed], [C1], [C2], 
[C3], [C4], [C5], [C6], [C7], [C8], [C9], [C10], [SQLTransferDTime], [SQLRowCounter], 
[RowSeqNum_IO]
From vWEM_MTECHLOADORDERS 
WHERE RowSeqNum_IO > (Select Max(RowSeqNum_IO) from SolomonApp.dbo.MTECHLOADORDERS WHERE [Feed Mill Code]='001327')
and [Feed Mill Code]='001327'
