...
-- =============================================
CREATE PROCEDURE [dbo].[cfp_PigInvDiff_old] 
...
From 
       ...
       left outer join
                                                   (select PIT.*, WD.FiscalPeriod, WD.FiscalYear, WD.PICWeek, WD.PICYear, WD.WeekOfDate, WD.WeekEndDate 
                                                    ...
                                                    Where PIT.Reversal<>1 and PIT.TranDate <= WD.WeekEndDate) TR ON pg.PigGroupID = TR.PigGroupID
      WHERE pg.piggroupid = @piggroupid and 
                                TR.WeekEndDate < GETDATE()+7  
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_PigInvDiff_old] TO [ASchimmelpfennig]
    AS [dbo];
