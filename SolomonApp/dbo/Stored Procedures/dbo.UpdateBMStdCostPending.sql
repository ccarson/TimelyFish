 Create Proc UpdateBMStdCostPending @KitKitID varchar (30), @KitSiteID varchar (10), @InvtIDNoKits varchar ( 30),
...
BEGIN
   ...
   SELECT k.KitID, k.SiteID, i.InvtType, i.InvtAcct, i.InvtSub
     FROM Kit k WITH(NOLOCK) JOIN Inventory i WITH(NOLOCK)
                  ON i.InvtId = k.KitId
...
BEGIN
      ...
      SELECT i.InvtID, s.SiteId, i.InvtType, i.InvtAcct, i.InvtSub
        ...
        WHERE i.InvtId like @InvtIDNoKits
            AND i.ValMthd = 'T'
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[UpdateBMStdCostPending] TO [MSDSL]
    AS [dbo];
