...
CREATE PROCEDURE XDDARDoc_Details_EBFileNbr_1
   @EBFileNbr		varchar( 6 ),
   @BatNbr		varchar( 10 ),	
   	...
   	if @Order = 'I'
	   	...
	   	FROM		XDDEBFile XF (nolock) LEFT OUTER JOIN XDDBatch XB (nolock)
				ON XF.EBFileNbr = XB.EBFileNbr and XF.FileType = XB.FileType LEFT OUTER JOIN ARDoc A (nolock)
	   			ON XB.BatNbr = A.BatNbr and XB.BatSeq = A.BatSeq LEFT OUTER JOIN XDDBatchAREFT B (nolock) 
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[XDDARDoc_Details_EBFileNbr_1] TO [MSDSL]
    AS [dbo];
