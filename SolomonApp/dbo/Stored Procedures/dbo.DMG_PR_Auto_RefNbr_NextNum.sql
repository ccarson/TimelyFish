 CREATE PROCEDURE DMG_PR_Auto_RefNbr_NextNum(
	...
	WHILE @Success = 0 AND @NumTries < 100 BEGIN
		...
		SELECT  @LastNbr     = LastRefNbr,
			@LastNbrLen  = DATALENGTH(RTRIM(LastRefNbr)),
			@tstamp      = tstamp
		FROM    APSetup
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[DMG_PR_Auto_RefNbr_NextNum] TO [MSDSL]
    AS [dbo];
