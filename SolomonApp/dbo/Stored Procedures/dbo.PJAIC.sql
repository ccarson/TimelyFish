...
-- The cpnyid, period and rate calculation type parameters are required. The other parameters allow a
-- degree of filtering. If they are passed as an empty string or DEFAULT, then all values will be
...
-- Dependent on: PJfINDRTS(), PJvAIC_source, PJvPJACCT_IndirectGrps
...
-- may be 'variable' source costs that need to be generated.

;with ics_cte (
    [level],
    anchor_alloc_method_cd,
    anchor_emp_cpnyid,
    anchor_emp_gl_subacct,
    anchor_pjt_entity,
    anchor_project,
    anchor_src_acct,
    anchor_src_cpnyid,
    anchor_src_gl_subacct,
    anchor_step_number,
...
select 1,
    ...
    -- If the indirect group has no rates, then zero is used.
...
where len(rtrim(case @ratecalctype when 'P' then s.post_acct_ptd_indirectgrp else s.post_acct_ytd_indirectgrp end)) > 0
  -- apply the year, project, and task criteria
  and s.fsyear_num = left(@period, 4)
...
select ics_cte.level + 1,
    ...
    ics_cte.anchor_emp_cpnyid,
    ics_cte.anchor_emp_gl_subacct,
    ics_cte.anchor_pjt_entity,
    ics_cte.anchor_project,
    ics_cte.anchor_src_acct,
    ics_cte.anchor_src_cpnyid,
    ics_cte.anchor_src_gl_subacct,
...
inner join ics_cte
 on ics_cte.alloc_calc_type in ('SH', 'SP')    -- It's a subtotal step so there are potentially some variable amounts
  ...
  and ics_cte.anchor_src_cpnyid = s.src_cpnyid
  and ics_cte.anchor_src_gl_subacct = s.src_gl_subacct
  and ics_cte.anchor_emp_cpnyid = s.emp_cpnyid
  and ics_cte.anchor_emp_gl_subacct = s.emp_gl_subacct
...
-- Ok, now sum up and calculate the actual indirect cost amounts, apply masks, etc.
-- NOTE: When task subaccount is empty, it uses the project subaccount.
select
    amount_01 = round(ics_sum.amount_01, cr.basecurydecpl),
    amount_02 = round(ics_sum.amount_02, cr.basecurydecpl),
    amount_03 = round(ics_sum.amount_03, cr.basecurydecpl),
    amount_04 = round(ics_sum.amount_04, cr.basecurydecpl),
    amount_05 = round(ics_sum.amount_05, cr.basecurydecpl),
    amount_06 = round(ics_sum.amount_06, cr.basecurydecpl),
    amount_07 = round(ics_sum.amount_07, cr.basecurydecpl),
    amount_08 = round(ics_sum.amount_08, cr.basecurydecpl),
    amount_09 = round(ics_sum.amount_09, cr.basecurydecpl),
    amount_10 = round(ics_sum.amount_10, cr.basecurydecpl),
    amount_11 = round(ics_sum.amount_11, cr.basecurydecpl),
    amount_12 = round(ics_sum.amount_12, cr.basecurydecpl),
    amount_13 = round(ics_sum.amount_13, cr.basecurydecpl),
    amount_14 = round(ics_sum.amount_14, cr.basecurydecpl),
    amount_15 = round(ics_sum.amount_15, cr.basecurydecpl),
    ics_sum.credit_cpnyid,
    ics_sum.credit_gl_acct,
    ics_sum.credit_gl_subacct,
    ics_sum.debit_cpnyid,
    ics_sum.debit_gl_acct,
    ics_sum.debit_gl_subacct,
    ics_sum.offset_acct,
    ics_sum.offset_pjt_entity,
    ics_sum.offset_project,
    ics_sum.pjt_entity,
    ...
    ics_sum.project,
    ics_sum.post_acct,
    ics_sum.post_pjt_entity,
    ics_sum.post_project
from (select
          ...
          credit_cpnyid = dbo.PJfMask_cpnyid(a.credit_cpnyId, ics_cte.anchor_src_cpnyid, ics_cte.anchor_emp_cpnyid, p.CpnyId),
          a.credit_gl_acct,
          credit_gl_subacct = dbo.PJfMask_gl_subacct(a.credit_gl_subacct, ics_cte.anchor_src_gl_subacct, ics_cte.anchor_emp_gl_subacct, p.gl_subacct,
              case when len(rtrim(t.pe_id01)) = 0 then p.gl_subacct else t.pe_id01 end),
          debit_cpnyid = dbo.PJfMask_cpnyid(a.debit_cpnyId, ics_cte.anchor_src_cpnyid, ics_cte.anchor_emp_cpnyid, p.CpnyId),
          a.debit_gl_acct,
          debit_gl_subacct = dbo.PJfMask_gl_subacct(a.debit_gl_subacct, ics_cte.anchor_src_gl_subacct, ics_cte.anchor_emp_gl_subacct, p.gl_subacct,
              case when len(rtrim(t.pe_id01)) = 0 then p.gl_subacct else t.pe_id01 end),
          offset_acct = dbo.PJfMask_acct(a.offset_acct, ics_cte.anchor_src_acct),
          offset_pjt_entity = dbo.PJfMask_pjt_entity(a.offset_pjt_entity, ics_cte.anchor_pjt_entity),
          offset_project = dbo.PJfMask_project(a.offset_project, ics_cte.anchor_project),
          pjt_entity = ics_cte.anchor_pjt_entity,
          p.ProjCuryId,
          p.ProjCuryRateType,
          project = ics_cte.anchor_project,
          post_acct = dbo.PJfMask_acct(a.post_acct, ics_cte.anchor_src_acct),
          post_pjt_entity = dbo.PJfMask_pjt_entity(a.post_pjt_entity, ics_cte.anchor_pjt_entity),
          post_project = dbo.PJfMask_project(a.post_project, ics_cte.anchor_project)
      from ics_cte
      ...
      inner join PJPENT t with (nolock)
       on t.project =  ics_cte.anchor_project
        and t.pjt_entity = ics_cte.anchor_pjt_entity
      ...
      group by p.CpnyId,
              ...
              dbo.PJfMask_project(a.post_project, ics_cte.anchor_project),
              dbo.PJfMask_pjt_entity(a.post_pjt_entity, ics_cte.anchor_pjt_entity),
              dbo.PJfMask_cpnyid(a.credit_cpnyId, ics_cte.anchor_src_cpnyid, ics_cte.anchor_emp_cpnyid, p.CpnyId),
              a.credit_gl_acct,
              dbo.PJfMask_gl_subacct(a.credit_gl_subacct, ics_cte.anchor_src_gl_subacct, ics_cte.anchor_emp_gl_subacct, p.gl_subacct,
                  case when len(rtrim(t.pe_id01)) = 0 then p.gl_subacct else t.pe_id01 end),
              dbo.PJfMask_acct(a.offset_acct, ics_cte.anchor_src_acct),
              dbo.PJfMask_pjt_entity(a.offset_pjt_entity, ics_cte.anchor_pjt_entity),
              dbo.PJfMask_project(a.offset_project, ics_cte.anchor_project),
              dbo.PJfMask_cpnyid(a.debit_cpnyId, ics_cte.anchor_src_cpnyid, ics_cte.anchor_emp_cpnyid, p.CpnyId),
              a.debit_gl_acct,
              dbo.PJfMask_gl_subacct(a.debit_gl_subacct, ics_cte.anchor_src_gl_subacct, ics_cte.anchor_emp_gl_subacct, p.gl_subacct,
                  case when len(rtrim(t.pe_id01)) = 0 then p.gl_subacct else t.pe_id01 end)) ics_sum
