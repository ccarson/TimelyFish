...
BEGIN
	SELECT @szWhereGeoZone = " AND EXISTS (SELECT e.EmployeeID
					FROM smEmpArea e, smSCQGeo g
					WHERE g.ConfigCode = '" + @parm3 + "' AND
					      e.AreaID = g.GeographicID AND
					      e.EmployeeID = smEmp.EmployeeID )"
END
IF @parm4 = '0'
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[smEmp_EMPIDFILTER] TO [MSDSL]
    AS [dbo];
