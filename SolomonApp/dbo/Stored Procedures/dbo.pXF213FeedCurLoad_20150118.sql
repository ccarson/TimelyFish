...
CREATE   Procedure [dbo].[pXF213FeedCurLoad_20150118]
	@MillID AS varchar(6), @Site As varchar(6), @parmdate smalldatetime, @Sort As varchar(1), @parmbio As varchar(1)

AS
Select fl.*, c.*, fo.*, tr.*, c2.*,cmt.*,(mm.OneWayHours*2) As TravelHours
...
--CASE WHEN @Sort='D' THEN  (CAST(mm.OneWayHours AS CHAR))
--END)
