...
CREATE procedure [dbo].[cfp_REPORT_INTERSTATE_PIG_MOVEMENTS]
(	@StartDate datetime
,	@EndDate datetime
...
IF @ShowChanges = 1
begin
	set @ShowChangesJoin = 'join (select distinct PMID, PMLoadID, PrevMovementDate from dbo.cftPMChanges (NOLOCK) '
	+ 'where PrevMovementDate between ''' + cast(@StartDate as varchar) + ''' and ''' + cast(@EndDate as varchar) + ''') PMC '
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[cfp_REPORT_INTERSTATE_PIG_MOVEMENTS] TO [MSDSL]
    AS [dbo];
