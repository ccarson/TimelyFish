 create proc ADG_CrtPrtQueue_Shipper
	@ri_id		smallint,
	@function	varchar(8),
	@class		varchar(4),
	@cpnyid		varchar(10),
...
as
	...
	BEGIN
		...
		from	SOShipHeader
		Where	CpnyID = @CpnyID
		and	ShipperID = @ShipperID and (ConsolInv = 0 or @function <> '4068000' and (NextFunctionID <> '4068000' or @function <> '%'))
	...
	BEGIN
		...
		and	ShipperID like @ShipperID and (ConsolInv = 0 or @function <> '4068000' and (NextFunctionID <> '4068000' or @function <> '%'))
		and	Status like @Status
		and	NextFunctionID like @Function
		and	NextFunctionClass like @Class
	END
	...
	left join SOPrintQueue q on SOShipHeader.cpnyid = q.cpnyid and SOShipHeader.shipperid = q.shipperid and SOPrintQueue.RI_ID = q.RI_ID '
	...
	select @WhereStr1 = 'where soprintqueue.ri_id = ' + Convert(varchar(6), @ri_id)
	
	if @shipperid = '%'
		select @WhereStr1 = @WhereStr1 +
			' and ((s.creditchk = 0 or SOShipHeader.creditchk = 0 or SOShipHeader.credithold = 0) and SOShipHeader.AdminHold = 0)
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[ADG_CrtPrtQueue_Shipper] TO [MSDSL]
    AS [dbo];
