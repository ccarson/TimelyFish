 CREATE procedure DMG_PR_Auto_RcptNbr_OP(
	@CpnyID   	CHAR(10),
	@NumTries 	SMALLINT,
	...
	WHILE @Done = 0 AND @Counter < @NumTries BEGIN
		...
		EXEC DMG_PR_Auto_RcptNbr_NextNum @Seq OUTPUT, @CpnyID
		...
		IF NOT EXISTS( SELECT RcptNbr FROM POReceipt (NOLOCK) WHERE RcptNbr = @DocID)
			SELECT @Done = 1
	END
