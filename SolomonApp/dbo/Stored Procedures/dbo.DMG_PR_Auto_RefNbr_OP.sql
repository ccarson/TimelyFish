 CREATE procedure DMG_PR_Auto_RefNbr_OP(
	@CpnyID   	CHAR(10),
	@NumTries 	SMALLINT,
	...
	WHILE @Done = 0 AND @Counter < @NumTries BEGIN
		...
		EXEC DMG_PR_Auto_RefNbr_NextNum @Seq OUTPUT, @CpnyID
		...
		IF NOT EXISTS( SELECT RefNbr FROM APDoc (NOLOCK) WHERE RefNbr = @DocID)
			SELECT @Done = 1
	END
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[DMG_PR_Auto_RefNbr_OP] TO [MSDSL]
    AS [dbo];
