...
CREATE PROC WSL_ProjectRateKeys_OutPut @ProjParm AS VARCHAR(16), @LEVEL VARCHAR(2) OUTPUT, @KEY1 VARCHAR(255) OUTPUT, @KEY2 VARCHAR(255) OUTPUT, 
                                       @KEY3 VARCHAR(255) OUTPUT, @RateTableID VARCHAR(4) OUTPUT, @RateType VARCHAR(2) OUTPUT
AS
    ...
    SELECT @PJR_Parm2 = RIGHT(Rtrim(control_data), 1)
    FROM   PJCONTRL
    ...
    SET @LEVEL = @Select_Level 
    SET @KEY1 = Isnull(@KeyVal_1, '-1')
    SET @KEY2 = Isnull(@KeyVal_2, '-1')
    SET @KEY3 = Isnull(@KeyVal_3, '-1')
    SET @RateTableID = @PJR_Parm1
    SET @RateType = @PJR_Parm2



GO
GRANT CONTROL
    ON OBJECT::[dbo].[WSL_ProjectRateKeys_OutPut] TO [MSDSL]
    AS [dbo];
