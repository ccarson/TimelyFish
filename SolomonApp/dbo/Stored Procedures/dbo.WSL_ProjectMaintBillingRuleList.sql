...
CREATE PROCEDURE [dbo].[WSL_ProjectMaintBillingRuleList]
 @page  int
 ,@size  int
 ,@sort   nvarchar(200)
...
AS
  ...
  IF @page = 0  -- Don't do paging
	  BEGIN
		SET @STMT = 
			'SELECT Code_Value [' + @Code_ValueAlias + '], code_value_desc [' + @code_value_descAlias + '], code_type [' + @code_typeAlias + ']
			 FROM PJCODE (nolock)
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[WSL_ProjectMaintBillingRuleList] TO [MSDSL]
    AS [dbo];
