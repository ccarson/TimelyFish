...
-- Updated 3/13/2015 BMD allow CF projects to load without a cftsite/contact record
-- Updated 8/07/2015 TNT include SiteIDs 8000 and 8010 in the result set
-- Updated 11/19/2015 TNT fixed issue with Sky not accepting 5 digits in the account number
-- ===================================================================
CREATE PROCEDURE [dbo].[cfp_CRM_project_services_extract]
...
AND s.SiteID not in ('9999','1660','0001')

union
/* select CF accounts including any account that does not match with data in the cftcontact table*/
select p.project as accountnumber
,replicate('0', 6 - len(c.contactid)) + cast (c.contactid as varchar) as 	cf_CDB_Contactid
, null cf_CDB_SiteId, 'Project' as type, p.lupd_datetime, p.customer as f1_billingaccount, 
	   case when c.contactname is null then p.project_desc else c.ContactName end as name
	  ,a.address1,a.address2,a.township,a.zip,ph.phonenbr
      ,a.City, a.county, a.latitude, a.longitude, a.range, a.sectionnbr, a.state
...
left join solomonapp.dbo.cftcontact c (nolock)
	...
	on cast(c.contactid as int) = cast(substring(p.project,3,5) as int)
LEFT JOIN solomonapp.dbo.cftContactAddress ca on c.ContactID = ca.ContactID AND ca.AddressTypeID = 1 
LEFT JOIN solomonapp.dbo.cftAddress a on ca.AddressID = a.AddressID
left join solomonapp.dbo.cftcontactphone cp (nolock) on cp.contactid = c.contactid and cp.phonetypeid = 1
left join solomonapp.dbo.cftphone ph (nolock) on ph.phoneid = cp.phoneid
...
join centraldata.dbo.Site s (nolock)
	...
	on s.pigrelatedglcode = substring(p.project,3,5)
join solomonapp.dbo.cftcontact c (nolock)
	on c.contactid = s.contactid
LEFT JOIN solomonapp.dbo.cftContactAddress ca on c.ContactID = ca.ContactID AND ca.AddressTypeID = 1 
LEFT JOIN solomonapp.dbo.cftAddress a on ca.AddressID = a.AddressID
