 create proc ADG_SO40600Bin_wrk
	@ri_id		smallint
...
as
	...
	select 	distinct S.CpnyID, S.ShipperID, S.LineRef, S.WhseLoc, @ri_id, 1,
		'', '', '', '',
		'',
		'', '', 0, 0, 0,
		0, '', '', 0, 0,
	...
	Declare SO40600_wrk_cursor Insensitive Cursor For
		select 	RI_ID, CpnyID, ShipperID, LineRef, WhseLoc, Cntr
		from 	SO40600_wrk
		where	RI_ID = @ri_id
		Open SO40600_wrk_cursor
	fetch next from SO40600_wrk_cursor into @RI_ID, @CpnyID, @ShipperID, @LineRef, @WhseLoc, @Cntr
		...
		begin
			...
			begin
				...
				insert	into SO40600_Wrk
					...
					S4Future11, S4Future12,
					Qty00, Qty01, Qty02, Qty03, Qty04 )
				values 	(@CpnyID, @ShipperID, @LineRef, @WhseLoc, @ri_id, @RecCntr,
					@LotSerNbr, '', '', '',
					'',
					'', '', 0, 0, 0,
					0, '', '', 0, 0,
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[ADG_SO40600Bin_wrk] TO [MSDSL]
    AS [dbo];
