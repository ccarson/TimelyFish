...
CREATE PROCEDURE [dbo].[WSL_ProjectMaintListByProject]
 @page  int
 ,@size  int
 ,@sort   nvarchar(200)
	  ...
	  BEGIN
		...
		SET @STMT = 
			'SELECT distinct QQ_pjprojNamed.project, [project description], isnull([customer name],'''') as [customer name], [project mgr name], isnull([business mgr name],'''') as [business mgr name], [company], [start date], [end date], [project status], QQ_pjprojNamed.noteid, Comment
			 ...
			 QQ_pjprojNamed.project LIKE ' + quotename(@parm4,'''') + '
			 ORDER BY ' + @sort
	  END		 
  ELSE
	  BEGIN
...
GO
GRANT CONTROL
    ON OBJECT::[dbo].[WSL_ProjectMaintListByProject] TO [MSDSL]
    AS [dbo];
