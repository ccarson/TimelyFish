 --APPTABLE
--USETHISSYNTAX

CREATE PROCEDURE pb_03800 @RI_ID SMALLINT

AS

DECLARE @RI_Where varchar(1024), @Search varchar(1024), @Pos SMALLINT,
	@BegPerNbr VARCHAR(6), @EndPerNbr VARCHAR(6), @ReportNbr CHAR(5)

SELECT @RI_Where = LTRIM(RTRIM(RI_Where)), @BegPerNbr = BegPerNbr, @EndPerNbr = EndPerNbr, @ReportNbr = ReportNbr
FROM RptRunTime
WHERE RI_ID = @RI_ID

IF @ReportNbr = '03800' 
    SELECT @Search = "({RptCompany.RI_ID} = " + RTRIM(CONVERT(VARCHAR(6),@RI_ID)) + ")"
ELSE
    SELECT @Search = "({vr_03820.RI_ID} = " + RTRIM(CONVERT(VARCHAR(6),@RI_ID)) + ")"

SELECT @Pos = PATINDEX("%" + @Search + "%", @RI_Where)

UPDATE RptRunTime SET RI_Where = CASE
	WHEN @RI_Where IS NULL OR DATALENGTH(@RI_Where) <= 0
		THEN @Search
	WHEN @Pos <= 0
		THEN @Search + " AND (" + @RI_WHERE + ")"
END
WHERE RI_ID = @RI_ID


