...
/****** Object:  View dbo.cfv_PigGroupSummaryBase    Script Date: 9/26/2005 2:23:53 PM ******/

/****** Sue Matter:  Used on CF663 Active Site Report    Script Date: 11/19/2004 2:10:22 PM ******/
CREATE     VIEW cfv_PigGroupSummaryBase
AS
SELECT    pj.pe_id05 As Company, ph.PhaseDesc, sd.FacilityType, pst.Description, pg.PigGroupID As GroupID, pj.pjt_entity_desc As GroupDesc, pg.PigGenderTypeID As Gender, CONVERT(CHAR(10),st.StartDate,101)As StartDate, 
DATEDIFF(day, fp.FirstOrder,st.StartDate) AS PrePig, DateDiff(day,st.StartDate, getdate()) as DaysIn, 
	  ...
	  sl.Qty As NoSold, CONVERT(NUMERIC(10,2),sl.TWgt/sl.Qty) As AvgSaleWgt, pg.SiteContactID, sv.ContactName, pg.PigSystemID, pd.Description As Pod, pg.CF04
FROM      cftPigGroup pg
