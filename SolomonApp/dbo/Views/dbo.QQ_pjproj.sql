...
CREATE VIEW [QQ_pjproj]
AS
SELECT     P.project, P.project_desc AS [project description], P.status_pa AS [project status], 
                      P.MSPInterface AS [integrated with MSP], P.customer, CASE WHEN CHARINDEX('~', NAME) > 0 THEN CONVERT(CHAR(30), 
                      LTRIM(SUBSTRING(NAME, 1, CHARINDEX('~', NAME) - 1)) + ', ' + LTRIM(RTRIM(SUBSTRING(NAME, CHARINDEX('~', NAME) + 1, 30)))) 
                      ELSE NAME END AS [customer name], '(' + SUBSTRING(C.Phone, 1, 3) + ')' + SUBSTRING(C.Phone, 4, 3) 
                      ...
                      P.manager1 AS [project manager], CASE WHEN CHARINDEX('~', pjemploy.emp_name) > 0 THEN CONVERT(CHAR(30), 
                      LTRIM(SUBSTRING(pjemploy.emp_name, 1, CHARINDEX('~', pjemploy.emp_name) - 1)) + ', ' + LTRIM(RTRIM(SUBSTRING(pjemploy.emp_name, 
                      CHARINDEX('~', pjemploy.emp_name) + 1, 30)))) ELSE pjemploy.emp_name END AS [project mgr name], P.manager2 AS [business manager],
                       CASE WHEN CHARINDEX('~', pjemploy_1.emp_name) > 0 THEN CONVERT(CHAR(30), LTRIM(SUBSTRING(pjemploy_1.emp_name, 1, CHARINDEX('~', 
                      pjemploy_1.emp_name) - 1)) + ', ' + LTRIM(RTRIM(SUBSTRING(pjemploy_1.emp_name, CHARINDEX('~', pjemploy_1.emp_name) + 1, 30)))) 
                      ELSE pjemploy_1.emp_name END AS [business mgr name], B.approver AS [invoice approver], convert(date,P.start_date) AS [start date], 
