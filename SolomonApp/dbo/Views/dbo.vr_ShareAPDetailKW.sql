...
/****** Created by DMK on 11/23/98 ******/
/****** D/E 203109 Include 0 balance vouchers DMK ******/
/****** DE 205868:  Eliminate all negative references to PP's in Docbal & OrigDocAmt *****/

CREATE VIEW vr_ShareAPDetailKW AS
...
/***	PerEnt = CASE v.Ord WHEN 3 THEN j.PerAppl ELSE d.PerEnt END, *****/
	...
	OrigDocAmt = CASE v.Ord 
		...
		WHEN 1 THEN (CASE d.DocType 
                                WHEN 'PP' 
                              then d.docbal + isnull((SELECT j.adjamt from apadjust j where d.refnbr = j.adjdrefnbr and d.doctype = "PP"),0)
                               else d.origdocamt
                              end)
              	  * (CASE d.DocType WHEN "AD" THEN -1 ELSE 1 END)	
		WHEN 2 THEN j.AdjAmt * v.CNum
		WHEN 3 THEN j.AdjDiscAmt * (CASE j.AdjdDocType WHEN "AD" THEN 1 WHEN "PP" THEN 1 ELSE -1 END) END,
	DocBal = CASE v.Ord WHEN 1 THEN d.DocBal * (CASE d.DocType WHEN "AD" THEN -1 ELSE 1 END) ELSE 0 END,
	...
	CuryOrigDocAmt = CASE v.Ord 
		...
		WHEN 3 THEN j.CuryAdjdDiscAmt * (CASE j.AdjdDocType WHEN "AD" THEN 1 WHEN "PP" THEN 1 ELSE -1 END) END,
	CuryDocBal = CASE v.Ord WHEN 1 THEN d.CuryDocBal * (CASE d.DocType WHEN "AD" THEN -1 WHEN "PP" THEN -1 ELSE 1 END) ELSE 0 END,
...
WHERE d.Rlsed = 1 AND (
	(v.Ord = 1 AND ((d.DocBal <> 0) AND (d.s4future11 <> "VM") and (d.masterdocnbr = ' ')) OR (d.DocType IN ("VO", "AC", "AD","PP","VM"))) OR
