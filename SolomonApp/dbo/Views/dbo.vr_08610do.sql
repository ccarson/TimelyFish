...
SELECT r.RI_ID, r.ReportDate, Parent=d.RefNbr, Ord=1, cCuryID=c.CuryID,
	...
	d.CustID, 
	CName=c.Name,
	cStatus=c.Status, d.RefNbr, d.CuryID, 
        d.DueDate, d.DiscDate, d.DocDate, d.DocType,
	...
	DaysPastDue = CASE WHEN d.DocType IN ('CM', 'PA', 'PP') AND ARSetup.S4Future09=0 THEN 0 
                           ELSE DATEDIFF(DAY, CASE WHEN d.DocType IN ('CM', 'PA', 'PP') 
                                                    THEN d.DocDate 
                                                 ELSE d.DueDate END, r.ReportDate) 
                       END,
	Past00 = CASE WHEN d.DocType IN ('IN','DM','FI','NC','AD') THEN 1 ELSE -1 END * CASE
		WHEN DATEDIFF(Day, d.DocDate, r.ReportDate) <= s.AgeDays00 OR 
                       d.DocType IN ('CM', 'PA', 'PP') AND ARSetup.S4Future09=0 THEN d.DocBal
		ELSE 0 END,
	Past01 = CASE WHEN d.DocType IN ('IN','DM','FI','NC','AD') THEN 1 ELSE -1 END * CASE
		WHEN DATEDIFF(Day, d.DocDate, r.ReportDate) <= s.AgeDays01 AND 
			DATEDIFF(Day, d.DocDate, r.ReportDate) > s.AgeDays00 AND
	...
	Past02 = CASE WHEN d.DocType IN ('IN','DM','FI','NC','AD') THEN 1 ELSE -1 END * CASE
		WHEN DATEDIFF(Day, d.DocDate, r.ReportDate) <= s.AgeDays02 AND 
			DATEDIFF(Day, d.DocDate, r.ReportDate) > s.AgeDays01 AND
                        (d.DocType NOT IN ('CM', 'PA', 'PP') OR ARSetup.S4Future09=1) THEN d.DocBal
		ELSE 0 END,
	...
	cPast00 = CASE WHEN d.DocType IN ('IN','DM','FI','NC','AD') THEN 1 ELSE -1 END * CASE
		WHEN DATEDIFF(Day, d.DocDate, r.ReportDate) <= s.AgeDays00 AND
  ...
  FROM RptRuntime r INNER JOIN RptCompany y
                     ...
                     INNER JOIN (SELECT StmtCycleID, AgeDays00 = CONVERT(INT,AgeDays00), 
                                        AgeDays01 = CONVERT(INT,AgeDays01),
                                        AgeDays02 = CONVERT(INT,AgeDays02) 
                                   FROM ARStmt) s 
                             ON s.StmtCycleID=c.StmtCycleID 
                      LEFT JOIN Terms t 
                        ON d.Terms <> '' AND t.TermsID=d.Terms 
                     CROSS JOIN ARSetup (NOLOCK)
 WHERE r.ReportNbr= '08610' 
   AND d.Rlsed=1 AND d.CuryDocBal<>0
