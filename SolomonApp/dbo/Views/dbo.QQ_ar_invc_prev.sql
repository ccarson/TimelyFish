...
CREATE VIEW [dbo].[QQ_ar_invc_prev]
AS
SELECT
    a.BatNbr AS [Batch Number], a.Status AS [Document Status],a.RefNbr As [Reference Number], a.CustId AS [Customer ID],
    ...
    a.AcctNbr AS [Account Number],
    a.BatSeq AS [Batch Sequence], a.CmmnAmt AS [Commission Amount], a.CmmnPct [Commission Percent], a.ContractID, 
    ...
    a.CuryCmmnAmt AS [Currency Commission Amount], a.CuryDiscApplAmt AS [Currency Discount Applied Amount],
    ...
    convert(date,a.DueDate) AS [DueDate], 
    convert(date,a.LUpd_DateTime) AS [Last Update Date], a.LUpd_Prog AS [Last Updated Program], a.LUpd_User AS [Last Updated User], 
    a.NbrCycle AS [Number of Cycles], a.NoPrtStmt AS [Document Reversal], a.NoteId, a.OpenDoc AS [Open Document], 
    a.OrdNbr AS [Order Number],  
    ...
    a.User5, a.User6, convert(date,a.User7) AS [User7], convert(date,a.User8) AS [User8]
FROM    Ardoc a with (nolock)
        INNER JOIN customer c with (nolock) ON a.CustID = c.CustId
WHERE   a.DocType IN ('IN', 'DM', 'CM', 'CS')
        AND a.Rlsed = 0
