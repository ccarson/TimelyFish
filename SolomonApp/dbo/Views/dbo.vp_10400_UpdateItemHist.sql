...
CREATE VIEW vp_10400_UpdateItemHist
As
SELECT T.UserAddress, FiscYr, T.InvtId, T.SiteId,
	...
	YtdCOGS = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'CG') 
  	                        Then ((T.TranAmt * T.InvtMult) * -1) Else 0 End),
	YtdCostAdjd = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'AJ') 
                                 Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'AC') 
                                 Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'PI') 
	                            Then (T.TranAmt * T.InvtMult) Else 0 End),
	YtdCostIssd = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'RI') Then ((T.ExtCost * T.InvtMult) * -1)
			       WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'AS' and T.KitID <> '') 
	                         Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'II' and I.StkItem = 1)
  	                           Then ((T.TranAmt * T.InvtMult) * -1)  
                               When (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'II' and I.StkItem = 0)
                                   Then ((T.ExtCost * T.InvtMult) * -1)
                               Else 0 End),
	YtdCostRcvd = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr And T.TranType = 'RC') 
	                         Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr And T.TranType = 'AS' and T.KitID = '') 
   	                            Then (T.TranAmt * T.InvtMult) Else 0 End),
	YtdCostTrsfrIn = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr  AND T.TranType = 'TR' and T.ToSiteId = '') 
  	                               Then (T.TranAmt * T.InvtMult) Else 0 End),
	YtdCostTrsfrOut = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr  AND T.TranType = 'TR' and T.ToSiteId <> '') 
  	                                Then ((T.TranAmt * T.InvtMult) * -1) Else 0 End),
	YtdQtyAdjd = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'AJ') 
                                Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'AC') 
                                Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'PI') 
	                           Then (T.Qty * T.InvtMult) Else 0 End),
	YtdQtyIssd = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'RI') 
			        Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'AS' and T.KitID <> '') 
	                        Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'II') 
  	                           Then ((T.Qty * T.InvtMult) * -1) Else 0 End),
	YtdQtyRcvd = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr And T.TranType = 'RC') 
                                Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr And T.TranType = 'AS' and T.KitID = '') 
   	                           Then (T.Qty * T.InvtMult) Else 0 End),
	YtdQtySls = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'CM' ) 
                          Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'DM' ) 
                          Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'IN' ) 
	                           Then ((T.Qty * T.InvtMult) * -1) Else 0 End),
	YtdQtyTrsfrIn = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr  AND T.TranType = 'TR' and T.ToSiteId = '') 
  	                              Then (T.Qty * T.InvtMult) Else 0 End),
	YtdQtyTrsfrOut = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr  AND T.TranType = 'TR' and T.ToSiteId <> '') 
  	                               Then ((T.Qty * T.InvtMult) * -1) Else 0 End),
	YtdSls = SUM(CASE WHEN (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'CM') 
                            Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'DM') 
                            Or (RIGHT(RTRIM(T.PerPost),2) <= T.PerNbr AND T.TranType = 'IN') 
	                       Then ((T.TranAmt * T.InvtMult) * -1) Else 0 End)
        FROM vp_10400_UpdateItemHist_INTran T, INSetup S, Inventory I
/*        Where T.Rlsed = 0  And T.Invtid = I.Invtid */
        Where T.Invtid = I.Invtid
        Group By T.UserAddress, T.FiscYr, T.InvtId, T.SiteId, T.PerPost
