...
CREATE VIEW PJvJobSummaryPjtCury
AS
  SELECT dbo.PJACTSUM.acct                 AS acct,
         dbo.PJACTSUM.ProjCury_amount_01   AS amount_01,
         dbo.PJACTSUM.ProjCury_amount_02   AS amount_02,
         dbo.PJACTSUM.ProjCury_amount_03   AS amount_03,
         dbo.PJACTSUM.ProjCury_amount_04   AS amount_04,
         dbo.PJACTSUM.ProjCury_amount_05   AS amount_05,
         dbo.PJACTSUM.ProjCury_amount_06   AS amount_06,
         dbo.PJACTSUM.ProjCury_amount_07   AS amount_07,
         dbo.PJACTSUM.ProjCury_amount_08   AS amount_08,
         dbo.PJACTSUM.ProjCury_amount_09   AS amount_09,
         dbo.PJACTSUM.ProjCury_amount_10   AS amount_10,
         dbo.PJACTSUM.ProjCury_amount_11   AS amount_11,
         dbo.PJACTSUM.ProjCury_amount_12   AS amount_12,
         dbo.PJACTSUM.ProjCury_amount_13   AS amount_13,
         dbo.PJACTSUM.ProjCury_amount_14   AS amount_14,
         dbo.PJACTSUM.ProjCury_amount_15   AS amount_15,
         dbo.PJACTSUM.fsyear_num           AS fsyear_num,
         dbo.PJACTSUM.project              AS project,
         dbo.PJPROJ.project_desc           AS project_desc,
         dbo.PJPROJ.ProjCuryId             AS project_currency,
         ...
         0                                 AS eac_amount,
         0                                 AS eac_units,
         dbo.PJACTSUM.ProjCury_amount_bf   AS Beg_Amt,
  ...
  SELECT dbo.PJPTDSUM.acct                 AS acct,
         ...
         dbo.PJPROJ.contract,
         dbo.PJEMPLOY.emp_name             AS emp_name,
         dbo.PJPTDSUM.ProjCury_tot_bud_amt AS total_budget_amount,
         dbo.PJPTDSUM.total_budget_units,
         dbo.PJPTDSUM.ProjCury_eac_amount  AS eac_amount,
         ...
         pjproj.gl_subacct                 AS subacct,
         dbo.pjptdsum.ProjCury_com_amount  AS com_amount,
         pjacct.sort_num,
         PJJOBSUMMARYWrk.ri_id,
         dbo.pjproj.CpnyId,
         RptCompany.CpnyName
  ...
  WHERE  PJREPCOL.column_nbr <> SPACE(0)
  UNION
  SELECT dbo.PJPTDROL.acct                 AS acct,
         0                                 AS amount_01,
         0                                 AS amount_02,
         0                                 AS amount_03,
         0                                 AS amount_04,
         ...
         0                                 AS amount_14,
         0                                 AS amount_15,
         LEFT(dbo.pjpoolh.period, 4)       AS fsyear_num,
         dbo.PJPTDROL.project              AS project,
         dbo.PJPROJ.project_desc           AS project_desc,
         dbo.PJPROJ.ProjCuryId             AS project_currency,
         dbo.PJACCT.acct_type              AS acct_type,
         dbo.Customer.name                 AS Cust_Name,
         ...
         0                                 AS units_15,
         0                                 AS units_bf,
         dbo.PJPOOLH.grpid                 AS grpid,
         dbo.PJPOOLH.period                AS period_rate,
         dbo.PJPOOLH.rate_ytd              AS rate_ytd,
         dbo.PJPOOLH.rate_ptd              AS rate_ptd,
         dbo.pjacct.ca_id06                AS target_rt,
         SPACE(0)                          AS task,
         pjproj.gl_subacct                 AS subacct,
         SPACE(0)                          AS com_amount,
  ...
  FROM   dbo.PJPTDROL
         INNER JOIN dbo.PJJOBSUMMARYWrk
           ON dbo.PJPTDROL.project = dbo.PJJOBSUMMARYWrk.project
         LEFT OUTER JOIN dbo.PJACCT
           ON dbo.PJPTDROL.acct = PJACCT.acct
         LEFT OUTER JOIN dbo.PJPOOLH
           ON LEFT(dbo.PJACCT.CA_ID01, 6) = PJPOOLH.grpid
         LEFT OUTER JOIN dbo.PJREPCOL
           ON dbo.PJACCT.ACCT = dbo.PJREPCOL.ACCT
         LEFT OUTER JOIN dbo.PJPROJ
           ON dbo.PJPTDROL.project = dbo.PJPROJ.project
         INNER JOIN dbo.RptCompany
           ON dbo.PJPROJ.CpnyId = dbo.RptCompany.CpnyID
               AND dbo.PJJOBSUMMARYWrk.ri_id = dbo.RptCompany.RI_ID
         LEFT OUTER JOIN dbo.PJPROJEX
           ON dbo.PJPTDROL.project = dbo.PJPROJEX.project
         LEFT OUTER JOIN dbo.Customer
           ON dbo.PJPROJ.customer = dbo.Customer.CustId
         LEFT OUTER JOIN dbo.PJEMPLOY
           ON dbo.PJPROJ.manager1 = dbo.PJEMPLOY.employee
