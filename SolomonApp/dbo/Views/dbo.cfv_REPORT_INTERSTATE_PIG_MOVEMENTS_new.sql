...
--*************************************************************
CREATE VIEW 
	dbo.cfv_REPORT_INTERSTATE_PIG_MOVEMENTS_new
AS
...
SELECT  DISTINCT
	...
	pm.PigGenderTypeID,
	DestFarm= Case when o.SrcPMID is null then 	cd.ContactName
...
,	sma.ServiceManagerName as ServiceManager
...
--		LEFT JOIN cftContact v ON cast(v.ContactID as int) = dbo.getVetLastVisit_VetContactID(pm.SourceContactID, pm.MovementDate)
		OUTER APPLY(	SELECT TOP 1 
							ContactID 		= hat.VetContactID 
						  , ContactName		= vc.ContactName
						  , LastVisitDate 	= hat.ActualTestDate
						FROM
							dbo.cft_HAT_Vet_Schedule AS hat
						LEFT JOIN
							dbo.cftContact AS vc
								ON vc.ContactID = hat.VetContactID
						WHERE 
							hat.SiteContactID = pm.SourceContactID 
								AND hat.ActualTestDate !> pm.MovementDate 
						ORDER BY 
							ActualTestDate DESC ) AS v
		...
		OUTER APPLY 
			dbo.cftvf_GetSvcManagerNm ( pm.SourceContactID, pm.MovementDate ) AS sma
...
do.City, do.State, do.Zip, do.County,v.LastVisitDate,
