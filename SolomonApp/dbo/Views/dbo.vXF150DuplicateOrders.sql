...
CREATE view vXF150DuplicateOrders as 
...
JOIN
	(SELECT     ContactId,BinNbr,QtyOrd,DateSched,COUNT(OrdNbr) AS Expr1,InvtIDOrd
	FROM          dbo.cftFeedOrder
	GROUP BY ContactId,BinNbr,PigGroupId,InvtIdOrd,QtyOrd,DateSched,OrdType,InvtIDOrd
	HAVING      (COUNT(OrdNbr) > 1) and left(OrdType,1)<>'R'
				) dup 
ON f.ContactId = dup.ContactId 
AND f.BinNbr = dup.BinNbr 
AND f.DateSched = dup.DateSched 
AND f.QtyOrd = dup.QtyOrd
--and f.OrdNbr<>dup.RelOrdNbr
and f.InvtIDOrd=dup.InvtIDOrd
