...
-- =============================================
CREATE FUNCTION [dbo].[cffn_GROWTH_BY_FEED_CONSUMPTION] 
(@StartWeight Float, @FeedConsumed Float)
RETURNS Float
AS
BEGIN
...
Declare @BaseFeedConsumed Float
...
Set @BaseFeedConsumed = (Select TotalFeedConsumed From @BaseDay Where BaseDay = @TargetDay)

	Select @FinalWeight = 
		Case When @StartWeight > 400 Then Null
			When @FeedConsumed = 0 Then @StartWeight
		Else (Select Top 1 BaseWeight From @BaseDay Where TotalFeedConsumed <= (@BaseFeedConsumed+@FeedConsumed) Order By TotalFeedConsumed Desc) 
