...
CREATE  PROCEDURE [dbo].[cfp_cube_ASOLivG_cumclosed_eb]
					@LOG char(1)='Y'
AS
BEGIN
/*
...
SET @ProcessName        = 'cfp_cube_ASOLivG_cumclosed_eb'
SET @ProcessStatus      = 0
...
IF @LOG='Y' 
	...
	END
--#####################################################################################################################
-- Delete rows from table:  prior x months
--#####################################################################################################################
...
Delete from  [dbo].[cft_ASOLivG_cumclosed]
where time >= @startweek

--CREATE TABLE [dbo].[cft_ASOLivG_cumclosed](
...
insert into  [dbo].[cft_ASOLivG_cumclosed]
select
...
d.PICYear_Week Time,
case when IT.acct='PIG SALE' and pm.MarketSaleTypeID='10' then 'CUM TOP PIG SALE'
when IT.acct='PIG SALE' and pm.MarketSaleTypeID<>'10' then 'CUM CLOSE PIG SALE'
when IT.acct='PIG SALE' and pm.MarketSaleTypeID is null then 'CUM CLOSE PIG SALE'
else 'CUM '+rtrim(IT.acct)
end Account,
...
join [$(SolomonApp)].dbo.cftPGInvTran it with (nolock) 
on it.TranDate<=WD.WeekEndDate
join [$(SolomonApp)].dbo.cfvDayDefinition_WithWeekInfo d with (nolock)
on WD.WeekOfDate=d.DayDate
join [$(SolomonApp)].dbo.cftPigGroup pg with (nolock)
on pg.PigGroupID=it.PigGroupID
join [$(SolomonApp)].dbo.cfv_GroupStart st with (nolock)
on st.PigGroupID=pg.PigGroupID
join [$(SolomonApp)].dbo.cfvDayDefinition_WithWeekInfo d2 with (nolock)
on d2.DayDate=st.StartDate
join [$(SolomonApp)].dbo.cftPigProdPod PP with (nolock)
on PP.PodID = PG.PigProdPodID
join [$(SolomonApp)].dbo.cftPigSystem PS with (nolock)
on PS.PigSystemID = PG.PigSystemID
join [$(SolomonApp)].dbo.cftPigProdPhase P with (nolock)
on P.PigProdPhaseID = PG.PigProdPhaseID
join [$(SolomonApp)].dbo.cftPGStatus S with (nolock)
on S.PGStatusID = PG.PGStatusID
join [$(SolomonApp)].dbo.cftBarn B
on B.ContactID=PG.SiteContactID and B.BarnNbr=PG.BarnNbr
join [$(SolomonApp)].dbo.cftFacilityType FT
...
(select
PMLoadID,
min(MarketSaleTypeID) MarketSaleTypeID
...
-- WHERE
where it.Reversal=0
and d.DayDate <= PG.ActCloseDate
and PG.PGStatusID = 'I'
and ST.StartDate >='1/1/2006'
and d.picyear_week >= @startweek
...
SET @Comments = 'Completed successfully - [dbo].[cft_ASOLivG_cumclosed], '
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_cube_ASOLivG_cumclosed_eb] TO [SE\ssis_datareader]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_cube_ASOLivG_cumclosed_eb] TO [SSRS_operator]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_cube_ASOLivG_cumclosed_eb] TO [SE\ssis_datawriter]
    AS [dbo];
