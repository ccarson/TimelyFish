...
-- =============================================
CREATE PROCEDURE [dbo].[cfp_CLOSEOUT_ANALYSIS_TEST]
(
...
BEGIN
	...
	else sc.ContactName end) + ' ' + 'Barn' + ' ' + rtrim(Case when left(rtrim(pm.TranSubTypeID),1)='O' then pmol.SourceBarnNbr
	else pm.SourceBarnNbr end) as 'NameBarn#',
	...
	else psr.AvgWgt end end - 
	Case when pm.EstimatedWgt = '' then 0 else pm.EstimatedWgt end as 'deltawgt',
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_CLOSEOUT_ANALYSIS_TEST] TO [db_sp_exec]
    AS [dbo];
