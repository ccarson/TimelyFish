...
-- =============================================
CREATE PROCEDURE [dbo].[cfp_SOW_SELECTED_GILT_70]
(
...
BEGIN
	...
	GFlow.ContactName,
	GFlow.FlowName,
	Case when GFlow.Phase in ('SB','SG') then Sum(Movement.Movement)*0.70 else Sum(Movement.Movement) end as Movement,
	...
	Case when GFlow.Source in ('M3/M4','M5/M6') then 'Total (M3/M4 & M5/M6) - 70% Selection' else '' end as Source2,
	GL.Descr,
	GL.TranDesc,
	Sum(GL.Amt) AS 'Amt',
	GMove.Movement,
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_SOW_SELECTED_GILT_70] TO [db_sp_exec]
    AS [dbo];
