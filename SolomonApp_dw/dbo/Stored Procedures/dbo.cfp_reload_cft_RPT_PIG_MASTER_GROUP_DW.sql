...
CREATE PROCEDURE [dbo].[cfp_reload_cft_RPT_PIG_MASTER_GROUP_DW]
AS
BEGIN
...
truncate table  dbo.cft_RPT_PIG_MASTER_GROUP_DW

--------------------------------------------------------------------------
-- BASE INFO
--------------------------------------------------------------------------
insert into cft_RPT_PIG_MASTER_GROUP_DW
select
       [MasterGroup]
      ,[SiteContactID]
      ,[PigFlowID]
      ...
      ,[PICQuarter]
	  ,[euthanized_Qty] 
	  ,[Deadb4Grade_Death_Qty] 
	  ,[euthanized_Wt]
	  ,[Deadb4Grade_Death_Wt]
	  , FYPeriod, reportinggroupid
from  dbo.cft_PIG_Master_GROUP_ROLLUP mg (nolock)
