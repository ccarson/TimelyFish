...
-- 2015-02-23 smr - change dayson paylean
-- 2015-06-08 TNT - Removed OrdType RD per Ron Schoo
-- =============================================
CREATE PROCEDURE [dbo].[cfp_SOLOMON_SCHEDULE_TEST_pmloadid]
(
...
--	convert(time,pm.loadingtime,0) loadingtime
	...
	, isnull(PayLn.DaysonPaylean,0) DaysonPaylean
...
/* 20130805 sripley  replacing the code above */
	...
	left join   -- 20150223  daysonpaylean change/addition
	(Select f.piggroupid,
				...
				from (
					...
					AND InvtIdOrd <> 'RD' -- 20150608 Remove InvtIDOrd RD per Ron Schoo
					and Reversal=0
		 ...
		 on PayLn.pmid = pm.pmid and PayLn.piggroupid = pm.sourcepiggroupid and payln.daysonpaylean > -1
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_SOLOMON_SCHEDULE_TEST_pmloadid] TO PUBLIC
    AS [dbo];
