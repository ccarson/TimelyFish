...
CREATE  PROCEDURE [dbo].[cfp_Accounting_AR_Audit]
	@BgDateRng DATETIME,
	@EnDateRng DATETIME,
	@LOG char(1)='Y'
AS
...
SET @ProcessName        = 'cfp_Accounting_AR_Audit'
SET @ProcessStatus      = 0
...
(crtd_user char(10) null,
 crtd_inv_cnt int null,
 crtd_line_cnt int null)
 
create table #lupd_audit_log
(lupd_user char(10) null,
 lupd_inv_cnt int null,
 lupd_line_cnt int null)
...
select  t.Crtd_User, count(distinct(d.RefNbr)), count(t.lineref)
from [$(SolomonApp)].dbo.ARDoc d (nolock)
join [$(SolomonApp)].dbo.ARTran t (nolock) on d.batnbr = t.batnbr and d.refnbr = t.refnbr
where t.acct <> '20000'
...
select  t.LUpd_User, count(distinct(d.refnbr)), count(t.lineref)
from [$(SolomonApp)].dbo.ARDoc d (nolock)
join [$(SolomonApp)].dbo.ARTran t (nolock) on d.batnbr = t.batnbr and d.refnbr = t.refnbr
where t.acct <> '20000'
