...
-- Description:	Returns alL Flow in Selected Gilt System
-- =============================================
CREATE PROCEDURE [dbo].[cfp_SOW_SELECTED_GILT_FLOW]
(
	@StartPeriod		int
	,@EndPeriod			int
	,@Source			varchar(8)
...
BEGIN
	...
	SELECT	'%' as FlowName1, '--All--' as FlowName
	UNION ALL
	SELECT	DISTINCT

	Rtrim(GF.FlowName) as FlowName1, 
	Rtrim(GF.FlowName) as FlowName
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_SOW_SELECTED_GILT_FLOW] TO [db_sp_exec]
    AS [dbo];
