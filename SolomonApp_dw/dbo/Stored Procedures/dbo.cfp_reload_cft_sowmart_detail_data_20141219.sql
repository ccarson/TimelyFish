...
CREATE PROCEDURE [dbo].[cfp_reload_cft_sowmart_detail_data_20141219]
AS
BEGIN
...
join (select be.site_id, be.identity_id, be.event_type, min(be.eventdate) eventdate
      ...
      where be.deletion_date is null and be.event_type <>  295
...
-- deal with depop events Begin
 ...
 -- into [$(CFFDB)].dbo.sowmart_detail_data_insert
  ...
  left join [dbo].[cftDayDefinition_WithWeekInfo]	dm on dm.daydate = dateadd(dd,-119,a.eventdate)
