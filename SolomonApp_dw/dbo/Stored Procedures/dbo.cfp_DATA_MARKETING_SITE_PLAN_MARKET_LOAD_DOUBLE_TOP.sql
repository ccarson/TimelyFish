...
-- =============================================
CREATE PROCEDURE dbo.cfp_DATA_MARKETING_SITE_PLAN_MARKET_LOAD_DOUBLE_TOP 
	@PigGroupID Char(6) 
...
If @MaxExistingMarketSaleTypeID = 10
	...
	Set @GrowthDaysToSecondTop = Case When @TargetTopWeight <= @CurrentAverageWeight Then 0
		Else (Select Top 1 PigDay 
				...
				Where PigWeight*1.06 >= @TargetTopWeight
				Order By PigDay)
			-
			(Select Top 1 PigDay 
				From  dbo.cft_GROWTH_AND_FEED_CONSUMPTION_CURVE
	...
	While @HeavyLoads > 0
		...
		,DateAdd(dd,(@GrowthDaysToSecondTop+@GrowthDaysToCloseout),DateAdd(dd,DateDiff(dd,0,GetDate()),0))
...
If @MaxExistingMarketSaleTypeID In(20,25)
	Begin
	--Project Closeouts
	Set @GrowthDaysToCloseout = Case When @TargetCloseoutWeight <= @CurrentAverageWeight Then 0
		Else	(Select Top 1 PigDay 
...
If @MaxExistingMarketSaleTypeID = 0
	...
	Set @GrowthDaysToFirstTop = Case When @TargetTopWeight <= @CurrentAverageWeight Then 0
		Else (Select Top 1 PigDay 
				From  dbo.cft_GROWTH_AND_FEED_CONSUMPTION_CURVE
				Where PigWeight*1.12 >= @TargetTopWeight
				Order By PigDay)
			-
			(Select Top 1 PigDay 
				From  dbo.cft_GROWTH_AND_FEED_CONSUMPTION_CURVE
	...
	While @HeavyLoads > 0
		...
		Insert Into @MarketLoad
		Select 'Closeout','LoadID',@PigGroupID,@BarnNumber,'Projected'
		,DateAdd(dd,(@GrowthDaysToFirstTop+@GrowthDaysToSecondTop+@GrowthDaysToCloseout),DateAdd(dd,DateDiff(dd,0,GetDate()),0))
		,'Packer',(Floor(@CurrentAverageWeight/5))*5+15,Null,Null
		Set @HeavyLoads = @HeavyLoads-1
		End
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_DATA_MARKETING_SITE_PLAN_MARKET_LOAD_DOUBLE_TOP] TO [db_sp_exec]
    AS [dbo];
