...
/*
	3)	INSERT Triumph packer detail into temp storage
*/
IF OBJECT_ID( 'tempdb..#PackerDetail' ) IS NOT NULL
	DROP TABLE #PackerDetail ;

CREATE TABLE
	#PackerDetail( 
        ID          int             not null IDENTITY PRIMARY KEY NONCLUSTERED
	  , CarcassID	int				NOT NULL	
	  , PlantID		char(06)  		NOT NULL 
	  , KillDate	smalldatetime	NOT NULL	
	  , TattooNbr	char(06)		NOT NULL
	  , HotWgt		float 			NOT NULL
	  , LeanPct		float			NOT NULL ) ; 
...
UPDATE STATISTICS dbo.cft_PACKER_DETAIL WITH FULLSCAN ;
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_PACKER_DETAIL_INSERT] TO [db_sp_exec]
    AS [dbo];
