...
-- =============================================
CREATE PROCEDURE [dbo].[cfp_SOW_PL2_remove]
(
...
BEGIN
	...
	Where DRS.SiteID <> 8450
	and Case When WD.FiscalPeriod < 10 
	Then Rtrim(Cast(WD.FiscalYear as char))+'0'+Rtrim(Cast(WD.FiscalPeriod as char)) 
	else Rtrim(Cast(WD.FiscalYear as char))+Rtrim(Cast(WD.FiscalPeriod as char)) end between @StartPeriod2 and @EndPeriod
	...
	Where PL.GroupPeriod between @StartPeriod2 and @EndPeriod
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_SOW_PL2_remove] TO [db_sp_exec]
    AS [dbo];
