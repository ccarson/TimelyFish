...
CREATE PROCEDURE [dbo].[cfp_REPORT_INVENTORY_TURNS_pluscost]
	@StartFiscalYear INT
...
SELECT 
  ...
  , sum(itemsite.totcost) as ExtCost
  , case when avg(itemsite.avgcost) = 0 and avg(itemsite.stdcost) = 0 then 0
	     when avg(itemsite.avgcost) <> 0 and avg(itemsite.stdcost) = 0 then avg(itemsite.avgcost)
	     when avg(itemsite.avgcost) = 0 and avg(itemsite.stdcost) <> 0 then avg(itemsite.stdcost)
	     else avg(itemsite.stdcost)  
	end as UnitCost
...
LEFT OUTER JOIN [$(SolomonApp)].dbo.ItemCost ItemCost ON (ItemSite.InvtID=ItemCost.InvtID) AND (ItemSite.SiteID=ItemCost.SiteID) -- this is an empty table
