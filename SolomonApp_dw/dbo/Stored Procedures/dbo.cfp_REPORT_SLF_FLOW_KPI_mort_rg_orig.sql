...
--
-- =============================================
CREATE PROCEDURE [dbo].[cfp_REPORT_SLF_FLOW_KPI_mort_rg_orig]
...
, (sum(pigdeath_qty) + sum(pigdeathTD_qty) + sum(transportdeath_qty))/((sum(transferinwp_qty) + sum(transferin_qty) + sum(movein_qty) - sum(MoveOut_qty) - sum(transferout_qty)))*100 as NurMortpct	-- remove the pigdeathTD_qty
into #mortNUR 
FROM [dbo].[cft_SLF_ESSBASE_DATA]
where pg_week between @fpyweek_start and @fpyweek_end	-- 50 to 25 weeks
...
, (sum(pigdeath_qty) + sum(pigdeathTD_qty) + sum(transportdeath_qty))/((sum(transferinwp_qty) + sum(transferin_qty) + sum(movein_qty) - sum(MoveOut_qty) - sum(transferout_qty)))*100 as finmortpct	-- remove the pigdeathTD_qty
...
, (sum(pigdeath_qty) + sum(pigdeathTD_qty) + sum(transportdeath_qty))/((sum(transferinwp_qty) + sum(transferin_qty) + sum(movein_qty) - sum(MoveOut_qty) - sum(transferout_qty)))*100 as wtfMortpct	-- remove the pigdeathTD_qty
...
--, 100 * (cast(#FH.sumba as float) - (cast(mkt.amtsold as float))) / cast(#FH.sumba as float)  as [totapctmort]
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_REPORT_SLF_FLOW_KPI_mort_rg_orig] TO [db_sp_exec]
    AS [dbo];
