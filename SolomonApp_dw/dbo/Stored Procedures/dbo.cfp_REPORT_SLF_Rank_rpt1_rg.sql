...
-- =============================================
-- Author:		SRipley, dbo.cfp_REPORT_SLF_Rank_rpt1_rg
...
-- =============================================
CREATE PROCEDURE [dbo].[cfp_REPORT_SLF_Rank_rpt1_rg]
...
-- Identify the Phase
select top 1 @phase = phase, @actstartdate = mpg.actstartdate from cft_rpt_pig_master_group_dw mpg (nolock)
...
select top 1 @prior_wk = picyear_week, @prior_mg = mastergroup, @prior_pigflow = reportinggroupid, @prior_phase = phase 
, @prior_start = [cft_RPT_PIG_MASTER_GROUP_DW].actstartdate, @prior_close = [cft_RPT_PIG_MASTER_GROUP_DW].actclosedate
...
and [cft_RPT_PIG_MASTER_GROUP_DW].actclosedate <= @actstartdate
...
where mgr.actclosedate between @prior_start and @prior_close
  and phase = @prior_phase
  and reportinggroupid = @prior_pigflow
;


WITH R (sitecontactid,picyear_week, contactname, phase, reportinggroupid, AdjAverageDailyGain,
AdjFeedToGain,
Mortality,
DOTDIY, adjadgrank, adjferank, mortrank, dotdiyrank)
AS
(
Select mgr.sitecontactid,PICYear_Week, c.contactname,Phase, reportinggroupid,AdjAverageDailyGain,AdjFeedToGain,Mortality,DOTDIY,
Rank() over (Partition by Phase, reportinggroupid Order by AdjAverageDailyGain desc) as 'AdjADGRank',
Rank() over (Partition by Phase, reportinggroupid Order by AdjFeedToGain) as 'AdjFERank',
Rank() over (Partition by Phase, reportinggroupid  Order by Mortality) as 'MortRank',
Rank() over (Partition by Phase, reportinggroupid  Order by dotdiy+1) as 'DOTDIYRank'
from  dbo.cft_RPT_PIG_MASTER_GROUP_DW mgr
inner join [$(SolomonApp)].dbo.cftPigGroup cpg on cpg.PigGroupID=mgr.MasterGroup and cpg.PigProdPodID!=53 -- Ignore SBF pig groups
inner join [$(CentralData)].dbo.contact c (nolock) 
	on c.solomoncontactid = mgr.sitecontactid
...
inner join [$(SolomonApp)].dbo.cftPigGroup cpg on cpg.PigGroupID=rb.MasterGroup and cpg.PigProdPodID!=53 -- Ignore SBF pig groups
where rb.actclosedate >= @actstartdate
and  picyear_week <= @PicYear_Week
and phase = @phase
and reportinggroupid = @reportinggroupid
and rb.sitecontactid = @sitecontactid
group by rb.sitecontactid,rb.picyear_week, rb.phase, case when rb.sitecontactid = @sitecontactid then 1 else 0 end, reportinggroupid
...
Select 0 as ordgrp, case when mgr.sitecontactid = @sitecontactid and picyear_week = @picyear_week then 1 else 0 end, PICYear_Week,
case when phase = @phase then c.contactname
	 else c.contactname + ' (' + phase + ')'
end contactname,
Phase, reportinggroupid,
AdjAverageDailyGain,
AdjFeedToGain,
Mortality,
DOTDIY,
...
rb.Phase, rb.reportinggroupid,
rb.adjadg as AdjAverageDailyGain,
rb.adjfe as AdjFeedToGain,
rb.mort as Mortality,
cast(rb.dotdiy as float) DOTDIY,
cast(Rank() over (Partition by rb.Phase, rb.reportinggroupid Order by rb.adjadg desc) as varchar(10)) as 'AdjADGRank',
cast(Rank() over (Partition by rb.Phase, rb.reportinggroupid Order by rb.adjfe) as varchar(10))  as 'AdjFERank',
cast(Rank() over (Partition by rb.Phase, rb.reportinggroupid  Order by rb.mort) as varchar(10))  as 'MortRank',
cast(Rank() over (Partition by rb.Phase, rb.reportinggroupid  Order by rb.dotdiy+1) as varchar(10))  as 'DOTDIYRank'
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_REPORT_SLF_Rank_rpt1_rg] TO [CorpReports]
    AS [dbo];
