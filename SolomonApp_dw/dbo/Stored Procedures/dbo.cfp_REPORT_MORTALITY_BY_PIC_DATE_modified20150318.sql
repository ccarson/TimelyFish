...
CREATE PROCEDURE [dbo].[cfp_REPORT_MORTALITY_BY_PIC_DATE_modified20150318]
	@PicDate CHAR(6)
,	@NumWeeks int
AS
...
--UPDATE	#ReportData
--SET	CumulativeAVGMortalityPercentByPhaseAndRegionalManager = x.CumulativeAVGMortalityPercent
--FROM	#ReportData ReportData
--JOIN	(SELECT SrSvcManager, PigProdPhaseID, AVG(AVGMortalityPercent) CumulativeAVGMortalityPercent FROM #ReportData GROUP BY SrSvcManager, PigProdPhaseID) x
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_REPORT_MORTALITY_BY_PIC_DATE_modified20150318] TO [db_sp_exec]
    AS [dbo];
