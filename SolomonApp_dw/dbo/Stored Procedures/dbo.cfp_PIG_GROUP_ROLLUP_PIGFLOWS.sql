...
CREATE PROCEDURE [dbo].[cfp_PIG_GROUP_ROLLUP_PIGFLOWS]
AS
BEGIN
...
join (SELECT PigGroupID, COUNT(*) cnt FROM
            ...
            FROM  dbo.cft_PIG_FLOW_SOURCE_FARMS cft_PIG_FLOW_SOURCE_FARMS (NOLOCK)
            WHERE isnull(SourceContactID,0) NOT IN (4001,4002)) x
 --           WHERE SourceContactID NOT IN (4001,4002)) x		--20140401 sripley replaced as it was excluding data
            GROUP BY PigGroupID
...
--2014-02-05 smr  correcting assignment of reporting group, too many others were assigned.
-- master groups with multiple piggroups/taskid if having the same reporting groupid, but different pigflowid, should be assigned the reporting groupid not other.
update  dbo.cft_PIG_MASTER_GROUP_ROLLUP
set reportinggroupid = xx.reportinggroupID
from  dbo.cft_PIG_MASTER_GROUP_ROLLUP base
inner join 
(select distinct m.mastergroup, flow.reportinggroupid 
FROM  dbo.cft_PIG_MASTER_GROUP_ROLLUP m (NOLOCK)
JOIN (SELECT DISTINCT MasterGroup, PigFlowID, reportinggroupid FROM  dbo.cft_PIG_GROUP_ROLLUP (NOLOCK)) flow
	ON flow.MasterGroup = m.MasterGroup
join (SELECT MasterGroup, COUNT(distinct reportinggroupID) reportinggroupIDcnt FROM  dbo.cft_PIG_GROUP_ROLLUP (NOLOCK)
	  GROUP BY MasterGroup HAVING COUNT(distinct reportinggroupid) = 1) pigcnt
	on pigcnt.mastergroup = m.MasterGroup) xx
	on xx.mastergroup = base.mastergroup

-- 2014-02-05 smr replacing this original code
--update cft_PIG_MASTER_GROUP_ROLLUP
--set reportinggroupid = flow.reportinggroupID
--FROM  dbo.cft_PIG_MASTER_GROUP_ROLLUP cft_PIG_MASTER_GROUP_ROLLUP (NOLOCK)
--JOIN (SELECT pigflowid, reportinggroupid FROM [$(CFApp_PigManagement)].dbo.cft_PIG_FLOW (NOLOCK)) flow
...
update  dbo.cft_PIG_MASTER_GROUP_ROLLUP
set reportinggroupid = pigflowid
where reportinggroupid is null
