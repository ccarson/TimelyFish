...
--
-- =============================================
CREATE  PROCEDURE [dbo].[cfp_REPORT_SLF_rpt2_5_final_interval_ranking_old]
...
select phase, pigflowid, sitecontactid
...
select c.contactname, c.solomoncontactid as sitecontactid
	...
	, base.AdjFeedToGain
	, base.Mortality
	, Rank() over (Partition by base.Phase, base.pigflowid Order by AdjAverageDailyGain desc) as 'AdjADGRank'
	, Rank() over (Partition by base.Phase, base.pigflowid Order by AdjFeedToGain) as 'AdjFERank'
	, Rank() over (Partition by base.Phase, base.pigflowid  Order by Mortality) as 'MortRank'
...
select rnk.contactname, rnk.sitecontactid, rnk.phase, rnk.pigflowid
...
inner join (select phase,pigflowid, max(AdjADGRank) maxadg, max(AdjFERank) maxFE, max(MortRank) maxmort, count(1) TotComp 
		    from #slf_rankings
			group by phase,pigflowid) mx
	on mx.phase = rnk.phase and mx.pigflowid = rnk.pigflowid
where sitecontactid =  @sitecontactid
  and contactname <> ''


select pct.contactname, pct.sitecontactid, pct.phase + '-' + substring(pf.PigFlowDescription,1,7) as phaseflow, pct.adjadgrankpctile, pct.adjferankpctile, pct.mortrankpctile, pct.comborank, pct.TotComp
from #slf_pcttile pct
LEFT JOIN [CFApp_PigManagement].[dbo].[cft_PIG_FLOW] PF (NOLOCK)
	ON pf.pigflowid = pct.pigflowid
