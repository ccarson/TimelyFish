...
-- Changed: 4/8/2015 BMD
--			Nursery ADG formula changed from ( (#ess_nur.final_wgt - #ess_nur.init_wgt)/#ess_nur.pigdays ) + ( (50 - (#ess_nur.final_wgt/#ess_nur.final_qty)) * .005 ) as 'ADG Nursery'
--                                      to  ( (#ess_nur.final_wgt - #ess_nur.init_wgt)/#ess_nur.pigdays ) + ( (50 - (#ess_nur.final_wgt/#ESS_nur.TransferOut_Qty)) * .005 ) as 'ADG Nursery'
--          Added TransferOut_QTY to #ess_nur
-- =============================================
CREATE PROCEDURE [dbo].[cfp_REPORT_SLF_FLOW_KPI_adg_staged_rg]
...
select reportinggroupid
	...
	, SUM(TransferOut_Qty) TransferOut_Qty
into #ESS_nur
from [dbo].[cft_SLF_ESSBASE_DATA] 
 where pg_week between @w25 and @pg_week
  and phase = 'NUR'
...
, ( w25w0.TwgtM - 3*(w25w0.hs/(1-#ea.pwm_pct))  ) 
/ ( w25w0.pigdays + ( (w25w0.hs/(1-#ea.pwm_pct)) ) * ( (w48w23.wean_age*(1-#ea.pwm_pct) ) + ( (w48w23.wean_age/2)*(#ea.pwm_pct) )  )     ) as 'ADG rollup'
into #ESS_rollup
 from 
(
select reportinggroupid
...
, ( (#ess_nur.final_wgt - #ess_nur.init_wgt)/#ess_nur.pigdays ) + ( (50 - (#ess_nur.final_wgt/#ESS_nur.TransferOut_Qty)) * .005 ) as 'ADG Nursery'	--  ess_nur(26 weeks)
