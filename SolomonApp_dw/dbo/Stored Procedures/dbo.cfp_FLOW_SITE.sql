...
-- =============================================
CREATE PROCEDURE [dbo].[cfp_FLOW_SITE]
(
...
BEGIN
	...
	Case when dd.FiscalPeriod < 10 
	then Rtrim(CAST(dd.FiscalYear AS char)) + '0' + Rtrim(CAST(dd.FiscalPeriod AS char)) 
	else Rtrim(CAST(dd.FiscalYear AS char)) + Rtrim(CAST(dd.FiscalPeriod AS char)) end,
	pgr.Phase
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[cfp_FLOW_SITE] TO [db_sp_exec]
    AS [dbo];
