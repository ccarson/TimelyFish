...
CREATE  PROCEDURE [dbo].[xx_cfp_cube_KILLDBFinal2_20170630]
                              @LOG char(1)='Y'
AS
BEGIN
/*
...
--, IIf(Trim$([MktMgrContactID])="","No Market Manager",[ContactName]) AS MktManger
, CASE When Rtrim(ltrim(cftMktMgrAssign.MktMgrContactID)) = '' then 'No Market Manager' 
         When Rtrim(ltrim(cftMktMgrAssign.MktMgrContactID)) = '004795' then 'No Market Manager'    -- unassigned, unassigned contact 20150218 smr
       else cftContact.ContactName 
  end as MktManger
...
FROM 
([$(SolomonApp)].dbo.cftPigSale cftPigSale (nolock) 
LEFT JOIN [$(SolomonApp)].dbo.cftPigSale AS rev (nolock) ON cftPigSale.RefNbr = rev.OrigRefNbr) 
INNER JOIN [$(SolomonApp)].dbo.cftPigGroup cftPigGroup (nolock) ON cftPigSale.PigGroupID = cftPigGroup.PigGroupID
WHERE 
(
(
...
FROM [$(SolomonApp)].dbo.cftPigSale cftPigSale (nolock)
LEFT JOIN [$(SolomonApp)].dbo.cftPigSale AS reverse (nolock) ON cftPigSale.RefNbr = reverse.OrigRefNbr
...
AND cftPigSale.OrdNbr<>'005605'  --jlm 2/10/16  added this to exclude the PigSales to SBF which were Inventory of Hogs in Active Pig Groups.
...
-------------------------------------------------------------------------------
-- create  table#cftKillDBfinal2_stg
-------------------------------------------------------------------------------
--SET  @StepMsg = 'Create table #cftKillDBfinal2_stg'
--EXEC [$(CFFDB)].dbo.cfp_PrintTs @StepMsg;


IF (OBJECT_ID ('[$(CFFDB)].dbo.cftKillDBfinal2_stg')) IS NOT NULL
      TRUNCATE TABLE [$(CFFDB)].dbo.cftKillDBfinal2_stg
...
INSERT INTO [$(CFFDB)].dbo.cftKillDBfinal2_stg
SELECT cfvDayDefinition_WithWeekInfo.PICYear_Week,
      
CASE --WHEN ltrim(rtrim(#qryMaxMktMgrEffDate.ContrNbr)) = '000011' THEN ltrim(rtrim(cftContact.contactname))  -- 201602 jlm not needed as this is handled in the else statement 
       WHEN ltrim(rtrim(#qryMaxMktMgrEffDate.ContrNbr)) = '000016'  THEN ltrim(rtrim(cftContact.contactname))+' Cargill'
--    WHEN ltrim(rtrim(#qryMaxMktMgrEffDate.ContrNbr)) = '000023'  THEN ltrim(rtrim(cftContact.contactname))+' Exetare' -- 201402 smr exetare purchase
      WHEN ltrim(rtrim(#qryMaxMktMgrEffDate.ContrNbr)) = '000023'  THEN ltrim(rtrim(cftContact.contactname))+' Clarkfield'    -- 201403 smr exetare rename
--    These criteria are to tag the Seaboard farms 'SBF' to the Packer name  **BEGIN** -- 201602 jlm
      WHEN cftPSOrdHdr.PSOrdTYpe = 'O' and cftPSOrdHdr.Descr not like 'SBF%' THEN ltrim(rtrim(cftContact.contactname))+' Open'
      WHEN cftPSOrdHdr.PSOrdTYpe = 'O' and cftPSOrdHdr.Descr like 'SBF%' THEN ltrim(rtrim(cftContact.contactname))+' Open SBF'      
       WHEN cftPSOrdHdr.PSOrdTYpe <> 'O' and cftPSOrdHdr.Descr like 'SBF%' then ltrim(rtrim(cftContact.contactname))+' SBF'  
       WHEN ltrim(rtrim(#qryMaxMktMgrEffDate.ContrNbr)) = '000023' and cftPSOrdHdr.Descr like 'SBF%'
            THEN ltrim(rtrim(cftContact.contactname))+' Clarkfield SBF'
--    These criteria are to tag the Seaboard farms 'SBF' to the Packer name  **END** -- 201602 jlm                    
       ELSE ltrim(rtrim(cftContact.contactname)) end as Packer,               
CASE WHEN ltrim(rtrim(cftPSType.Descr)) = 'Butcher' Then 'Butcher Sale'
      WHEN ltrim(rtrim(cftPSType.SalesTypeId)) = 'MS' Then ltrim(rtrim(cftPSType.Descr))+ ' ' + ltrim(rtrim(#qryMaxMktMgrEffDate.SaleBasis))
      else ltrim(rtrim(cftPSType.Descr)) end as [Sales Type], 
T2.[MarketManager],
isnull(cftPigGenderType.Description,'No Gender') as Gender,
isnull(st.LoadType,'No LoadType') as LoadType,
...
FROM #qryMaxMktMgrEffDate 
      ...
      LEFT JOIN [$(SolomonApp)].dbo.cftContact cftContact (nolock) ON #qryMaxMktMgrEffDate.PkrContactId = cftContact.ContactID
      LEFT JOIN [$(SolomonApp)].dbo.cftPSOrdHdr cftPSOrdHdr (nolock) ON #qryMaxMktMgrEffDate.OrdNbr = cftPSOrdHdr.OrdNbr
      LEFT JOIN [$(SolomonApp)].dbo.cftPigGroup cftPigGroup (nolock) ON #qryMaxMktMgrEffDate.PigGroupID = cftPigGroup.PigGroupID
      LEFT JOIN [$(SolomonApp)].dbo.cftPSType cftPSType (nolock) ON #qryMaxMktMgrEffDate.SaleTypeId = cftPSType.SalesTypeId
      LEFT JOIN [$(SolomonApp)].dbo.cftPigGenderType cftPigGenderType ON cftPigGroup.PigGenderTypeID = cftPigGenderType.PigGenderTypeID
...
--                      inner join cftMarketSaleType MST on MST.MarketSaleTypeId = cftPM.MarketSaleTypeID) as cftPM2
      left join (select distinct pmid, marketsaletypeid from [$(SolomonApp)].dbo.cftPM (nolock)) as cftPM
                  on cftPM.PMID = #qryMaxMktMgrEffDate.PMLoadId
      left join 
      ...
      from [$(SolomonApp)].dbo.cftMarketSaleType (nolock)) as st
                  on st.marketsaletypeid = cftPM.MarketSaleTypeID
      LEFT JOIN [$(SolomonApp)].dbo.cfvDayDefinition_WithWeekInfo cfvDayDefinition_WithWeekInfo ON #qryMaxMktMgrEffDate.SaleDate = cfvDayDefinition_WithWeekInfo.DayDate
WHERE #qryMaxMktMgrEffDate.SaleDate >= '1/3/2010'
GROUP BY cfvDayDefinition_WithWeekInfo.PICYear_Week,
CASE WHEN ltrim(rtrim(#qryMaxMktMgrEffDate.ContrNbr)) = '000016'  THEN ltrim(rtrim(cftContact.contactname))+' Cargill'
      WHEN ltrim(rtrim(#qryMaxMktMgrEffDate.ContrNbr)) = '000023'  THEN ltrim(rtrim(cftContact.contactname))+' Clarkfield'
      WHEN cftPSOrdHdr.PSOrdTYpe = 'O' and cftPSOrdHdr.Descr not like 'SBF%' THEN ltrim(rtrim(cftContact.contactname))+' Open'
      WHEN cftPSOrdHdr.PSOrdTYpe = 'O' and cftPSOrdHdr.Descr like 'SBF%' THEN ltrim(rtrim(cftContact.contactname))+' Open SBF'      
       WHEN cftPSOrdHdr.PSOrdTYpe <> 'O' and cftPSOrdHdr.Descr like 'SBF%' then ltrim(rtrim(cftContact.contactname))+' SBF'  
       WHEN ltrim(rtrim(#qryMaxMktMgrEffDate.ContrNbr)) = '000023' and cftPSOrdHdr.Descr like 'SBF%'
...
isnull(st.LoadType,'No LoadType'),
#qryMaxMktMgrEffDate.CpnyID
;
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[xx_cfp_cube_KILLDBFinal2_20170630] TO [SE\ssis_datareader]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[xx_cfp_cube_KILLDBFinal2_20170630] TO [SSRS_operator]
    AS [dbo];
