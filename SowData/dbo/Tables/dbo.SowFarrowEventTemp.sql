CREATE TABLE [dbo].[SowFarrowEventTemp] (
    ...
    [SortCode]     INT           NULL,
    CONSTRAINT [PK_SowFarrowEventTemp] PRIMARY KEY CLUSTERED ([EventID] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE NONCLUSTERED INDEX [idxSowFarrowEventTemp_FarmIDSowID]
    ON [dbo].[SowFarrowEventTemp]([FarmID] ASC, [SowID] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idxSowFarrowEventTemp_EventDate]
    ON [dbo].[SowFarrowEventTemp]([EventDate] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idxSowFarrowEventTemp_WeekOfDate]
    ON [dbo].[SowFarrowEventTemp]([WeekOfDate] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idxSowFarrowEventTemp_SowParity]
    ON [dbo].[SowFarrowEventTemp]([SowParity] ASC) WITH (FILLFACTOR = 90);
