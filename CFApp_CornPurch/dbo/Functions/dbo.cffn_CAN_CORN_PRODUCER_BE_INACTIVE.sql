...
-- ===================================================================
CREATE FUNCTION [dbo].[cffn_CAN_CORN_PRODUCER_BE_INACTIVE] (
...
BEGIN 
  ...
  SELECT @Result = CASE WHEN EXISTS(
                                    ...
                                    LEFT OUTER JOIN dbo.cft_PARTIAL_TICKET PT (nolock) ON PT.CornProducerID = CP.CornProducerID
										and PT.PartialTicketID IS NOT NULL AND ISNULL(PT.SentToAccountsPayable, 0) = 0	-- 20130820 smr copied this condition statement here from the where clause
			/* the optimizer is not selecting the proper access path */
                                    --open contracts
                                    ...
                                    WHERE CP.CornProducerID = @CornProducerID
                                          AND (
                                                ...
                                                OR (PT.PartialTicketID IS NOT NULL AND ISNULL(PT.SentToAccountsPayable, 0) = 0)	-- 20130820 copied to outer join
